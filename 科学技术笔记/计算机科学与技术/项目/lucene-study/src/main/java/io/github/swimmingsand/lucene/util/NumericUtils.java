...
public final class NumericUtils {
  ...
  public static int sortableFloatBits(int bits) {
    // bits 是一个IEEE 754表示
    // 0x7fffffff = 0b01111111111111111111111111111111 31个1

    // 3.5
    // 0 10000000 11000000000000000000000 bits
    // 0 00000000 00000000000000000000000 bits >> 31
    // 0 10000000 11000000000000000000000 bits ^ (bits >> 31)
    // 0 10000000 11000000000000000000000 bits ^ (bits >> 31) & 0x7fffffff

    // 1 10000000 11000000000000000000000 bits
    // 0 11111111 11111111111111111111111 (bits >> 31) & 0x7fffffff
    // 1 01111111 00111111111111111111111 bits ^ (bits >> 31) & 0x7fffffff
  ...
  public static void main(String args[]) {
     ...
     */
    // 0 11111111 00000000000000000000000 EXP_BIT_MASK
    // 0 00000000 11111111111111111111111 SIGNIF_BIT_MASK
    // 0 11111111 10000000000000000000000 NaN
    // 0 11111111 00000000000000000000000 positive infinity
    // 1 11111111 00000000000000000000000 negative infinity
    int a = NumericUtils.floatToSortableInt(3.5f);
    System.out.println(a);

    int b = NumericUtils.floatToSortableInt(-3.5f);
    ...
    for (int i=0; i< 256;i++) {
      int signBits = (i >> 7) & 0x1;
      String signBitsStr = String.format("%1s", Integer.toBinaryString(signBits)).replace(" ", "0");

      int expBits = (i >> 3) & 0xF;
      String expBitsStr = String.format("%4s", Integer.toBinaryString(expBits)).replace(" ", "0");

      int fracBits = (i) & 0x7;
      String fracBitsStr = String.format("%3s", Integer.toBinaryString(fracBits)).replace(" ", "0");
      ...
      String toPrint = "";
      if (expBits == 0) {
        exp = (float) Math.pow(2, (1-bias));
        double fracTmp = 0.0 + ((fracBits >> 2) & 0x1) * 1/2.0 + ((fracBits >> 1) & 0x1) * 1/4.0 + ((fracBits >> 0) & 0x1) * 1/8.0;
        frac = (float)fracTmp;
        toPrint = signBitsStr + " " + expBitsStr + " " + fracBitsStr + " " + Math.pow(-1.0, signBits) * exp * frac + " " + floatToSortableInt((float)(Math.pow(-1.0, signBits) * exp * frac));
      } else if (expBits == 0xF) {
          if (fracBits == 0) {
            if (signBits == 0) {
              // +infinity
              toPrint = signBitsStr + " " + expBitsStr + " " + fracBitsStr + " +infinity";
            } else {
              // -infinity
              toPrint = signBitsStr + " " + expBitsStr + " " + fracBitsStr + " -infinity";
            }
          } else {
            toPrint = signBitsStr + " " + expBitsStr + " " + fracBitsStr + " NaN";
          }
      } else {
        exp = (float) Math.pow(2, (expBits-bias));
        double fracTmp = 1.0 + ((fracBits >> 2) & 0x1) * 1/2.0 + ((fracBits >> 1) & 0x1) * 1/4.0 + ((fracBits >> 0) & 0x1) * 1/8.0;
        frac = (float)fracTmp;
        toPrint = signBitsStr + " " + expBitsStr + " " + fracBitsStr + " " + Math.pow(-1.0, signBits) * exp * frac + " " + floatToSortableInt((float)(Math.pow(-1.0, signBits) * exp * frac));;
      }
